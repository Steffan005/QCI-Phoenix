...
mod heuristics;
use heuristics::HeuristicValidator;
...
/// Simplified - no sidecar spawning (services managed externally)
/// Just probe existing services
...
fn preflight() -> bool {
    ...
    let backend_ok = probe("http://127.0.0.1:8001/health", 30, 500);
    if !backend_ok {
...
#[tauri::command]
async fn validate_heuristics(text: String) -> Result<heuristics::HeuristicResult, String> {
    let validator = HeuristicValidator::new();
    Ok(validator.validate(&text))
}
...
fn main() {
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            println!("[Unity] Setup: Checking for running services...");

            // Run preflight checks in background (non-blocking)
            let app_handle = app.handle();
            tauri::async_runtime::spawn(async move {
                // Check if services are already running
                println!("[Unity] Checking for existing services...");
                thread::sleep(Duration::from_secs(1));

                // Run preflight
                if preflight() {
                    println!("[Unity] Preflight complete: Services detected and ready");
                    app_handle
                        .emit_all("unity:ready", "preflight_ok")
                        .ok();
                } else {
                    eprintln!("[Unity] Preflight: Services not detected - please start backend manually");
                    app_handle
