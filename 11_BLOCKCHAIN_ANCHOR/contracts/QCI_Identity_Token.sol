...
contract QCIGovernance is ERC20, ERC20Burnable, ERC20Votes, AccessControl, Pausable {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // LICENSE ENFORCEMENT - The Poison Pill
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    constructor(
        ...
        // ═══════════════════════════════════════════════════════════════════════
        // OPERATION IRON DOME - Sovereign Vault Allocations
    ...
    function coherenceMint(
        ...
        // THE EQUATION MUST MATCH - On-chain coherence verification
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // LICENSE ENFORCEMENT - The Poison Pill Functions
    // ═══════════════════════════════════════════════════════════════════════════

    /**
     * @notice Blacklist an address for license violation
     * @dev Only callable by admin - freezes all token operations for violator
    ...
    function setBlacklist(
        ...
        blacklisted[violator] = status;

        if (status) {
            emit LicenseRevoked(violator, reason, block.timestamp);
        } else {
            emit LicenseRestored(violator, block.timestamp);
        }
    }
    ...
    function _beforeTokenTransfer(
        ...
        // LICENSE ENFORCEMENT: Blacklisted addresses cannot send or receive
        require(!blacklisted[from], "QCI: Sender blacklisted for license violation");
        require(!blacklisted[to], "QCI: Recipient blacklisted for license violation");
...
contract QCIPhoenixProtocol is AccessControl, ReentrancyGuard, Pausable {
    ...
    mapping(address => bool) public protocolBlacklist;
    ...
    event ProtocolBlacklistUpdated(address indexed account, bool status, string reason);
    ...
    constructor(
        ...
        // Deploy governance token with sovereign vault allocations
    ...
    function claimAirdrop(bytes32[] calldata proof) external nonReentrant whenNotPaused {
        require(!protocolBlacklist[msg.sender], "QCI: Address blacklisted");
    ...
    /**
     * @notice Blacklist an address from protocol participation
     * @dev The Poison Pill - license enforcement at protocol level
     */
    function setProtocolBlacklist(
        ...
        protocolBlacklist[account] = status;
        emit ProtocolBlacklistUpdated(account, status, reason);
    }
